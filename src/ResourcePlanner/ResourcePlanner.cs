...
public class ResourcePlanner : IResourcePlanner
{
    ...
    private async Task<ActionModel> InferResource(ActionModel actionParam)
    {
        ...
        List<Middleware.ResourcePlanner.RedisInterface.ActivePolicy> activePolicies =
            (await redisApiClient.PolicyGetActiveAsync()).ToList();
        Dictionary<Guid, List<Guid>> tempDic =
            new Dictionary<Guid, List<Guid>>(); //Diccionary of Key policy ID, values List of results after query
    ...
    public async Task<TaskModel> Plan(TaskModel taskModel)
    {
        ...
        foreach (ActionModel action in actionSequence)
        {
            ...
            {
                RedisInterface.InstanceModel instanceTmp =
                    await redisApiClient.InstanceGetByIdAsync(relation.PointsTo.Id);
                //map
