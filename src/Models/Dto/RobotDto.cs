...
public class RobotDto : Dto
{
    ...
    [Indexed] [RedisIdField] public override string Id { get; set; }

    [Indexed] public string? Name { get; set; }

    [Indexed(JsonPath = "$.RosDistro")] public RosInfo Ros { get; init; } = new();

    [Indexed(Sortable = true)] public DateTimeOffset LastUpdatedTime { get; set; }

    [Indexed] public string? RobotModelName { get; set; }

    [Indexed] public string? RobotStatus { get; set; }

    [Indexed] public string? Manufacturer { get; set; }

    public Uri? ManufacturerUrl { get; set; }

    [Indexed] public long? MaximumPayload { get; set; }

    [Indexed] public long? MaximumTranslationalVelocity { get; set; }

    [Indexed] public string CurrentTaskId { get; set; }

    [Indexed] public List<string>? TaskList { get; set; }

    [Indexed] public string? LocomotionTypes { get; set; }

    public long? MaximumRotationalVelocity { get; set; }

    public long? RobotWeight { get; set; }

    [Indexed] public int BatteryStatus { get; set; }
    ...
    [Indexed] public List<Sensor>? Sensors { get; set; } = new();

    [Indexed] public List<Actuator>? Actuators { get; set; } = new();

    [Indexed] public List<Manipulator>? Manipulators { get; set; } = new();

    [Indexed] public List<DialogueModel>? Questions { get; init; } = new();

    [Indexed] public DateTimeOffset OnboardedTime { get; init; } = DateTimeOffset.Now;

    [Indexed] public List<string> QuestionIds { get; set; } = new();

    public HardwareSpec HardwareSpec { get; init; } = new();
    [Indexed] public string? SimCardNumber { get; init; }
